...
public class AdminServices : IAdminServices
{
    ...
    public async Task<Result<ProductDetailResponse>> CreateProduct( CreateProductRequest request )
    {
        ...
        try
        {
            ...
            // Auto-fetch specs and image from Icecat after product creation
            _ = Task.Run(async () =>
            {
                try { await _specsService.FetchFromIcecat(product.Id); }
                catch { /* fail silently */ }
            });
    ...
    public async Task<Result<UserDetailResponse>> GetUserById( int id )
    {
        try
        {
            ...
            var orders = user.Orders.Select(o => new OrderResponse(
                ...
                GrandTotal: o.TotalPrice + o.DeliveryPrice,
                PaymentMethod: o.PaymentMethod ?? "Card",
                IsPaid: o.IsPaid,
                TrackingNumber: o.TrackingNumber,
                TrackingNotes: o.TrackingNotes,
                EstimatedDeliveryDate: o.EstimatedDeliveryDate?.ToString("yyyy-MM-dd"),
                ShippedAt: o.ShippedAt?.ToString("yyyy-MM-dd"),
                DeliveredAt: o.DeliveredAt?.ToString("yyyy-MM-dd"),
                DeliveryMethod: null,
    ...
    public async Task<Result<AdminOrderResponse>> GetOrderById( int id )
    {
        try
        {
            ...
            var response = new AdminOrderResponse(
                ...
                GrandTotal: order.TotalPrice + order.DeliveryPrice,
