...
public class OrderService : IOrderService
{
    ...
    public async Task<Result<OrderResponse>> CreateOrder( int userId, CreateOrderRequest request )
    {
        ...
        try
        {
            ...
            // Get delivery method
            var deliveryMethod = request.DeliveryMethodId > 0
                ? await _DB.DeliveryMethods.FindAsync(request.DeliveryMethodId)
                : null;
            ...
            var order = new Entities.Order
            {
                ...
                IsPaid = request.PaymentMethod == "Card", // Card = instant simulated payment
                PaidAt = request.PaymentMethod == "Card" ? DateTime.UtcNow : null,
                EstimatedDeliveryDate = deliveryMethod != null
                    ? DateTime.UtcNow.AddDays(deliveryMethod.EstimatedDays > 0 ? deliveryMethod.EstimatedDays : 1)
